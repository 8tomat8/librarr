...
func Load(path, url string) *Registry {
	...
	r, err := Default()
	if err != nil {
		// This would only happen if the embedded JSON itself were malformed,
		// which is a build-time invariant. Return a non-nil empty registry
		// rather than panicking so callers can still range over slices.
